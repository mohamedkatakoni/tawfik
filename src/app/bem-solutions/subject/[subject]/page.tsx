import { bemSubjectScraper } from "@/scraper";
...
export default async function BemSubjectPage({ params }: PageProps) {
  const { subject } = await params;
  const data = await bemSubjectScraper(subject);
    ...
    <main className="min-h-screen font-['Tajawal']" style={{ background: "#F7F3EC" }}>
      ...
      <div className="max-w-5xl mx-auto px-6 lg:px-8 py-10">
        ...
        <div className="mb-6">
          ...
          >
            {data.title || `مواضيع شهادة التعليم المتوسط ${subjectName}`}
          </h1>
          {data.description && (
