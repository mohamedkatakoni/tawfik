...
export default async function BemSolutionsPage() {
  const { list } = await finalStageExamsBem("bem-solutions");
    ...
    >
      ...
      <div className="max-w-5xl mx-auto px-6 lg:px-8 py-10">
        ...
        <div className="mb-8">
          ...
          >
            مواضيع وحلول{" "}
            <span className="text-[#7C3AED] italic">BEM</span>
          </h1>
          <p className="text-xs max-w-md leading-relaxed" style={{ color: "#777" }}>
            جميع مواضيع وحلول شهادة التعليم المتوسط منذ 1996
          </p>
        </div>
        ...
        <div className="space-y-2 max-w-2xl">
          {list.map((item, index) => (
            <Link
              key={index}
              href={`/bem-solutions/${item.urlbem}`}
