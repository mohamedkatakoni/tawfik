...
const stageNames: Record<string, string> = {
  "first-primary": "السنة الأولى ابتدائي",
  "second-primary": "السنة الثانية ابتدائي",
  "third-primary": "السنة الثالثة ابتدائي",
  "fourth-primary": "السنة الرابعة ابتدائي",
  "fifth-primary": "السنة الخامسة ابتدائي",
};
...
export default async function MaterialPage({ params }: PageProps) {
  ...
  const categories = await specificMaterialPdfs("ens-pri", primarystage, material);
    ...
    <main className="min-h-screen font-['Tajawal']" style={{ background: "#F7F3EC" }}>
      <div className="max-w-4xl mx-auto px-6 py-10">
        ...
        <nav className="flex items-center justify-center gap-3 text-sm mb-10" style={{ color: "#999" }}>
          ...
          <Link href="/ens-pri" className="font-semibold hover:text-[#7C3AED] transition-colors">الابتدائي</Link>
          <ChevronLeft className="w-4 h-4" />
          <Link href={`/ens-pri/${primarystage}`} className="font-semibold hover:text-[#7C3AED] transition-colors">
            {stageName}
          </Link>
          <ChevronLeft className="w-4 h-4" />
