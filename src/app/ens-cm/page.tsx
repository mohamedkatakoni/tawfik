...
const stageMeta: Record<string, { num: string; bg: string; color: string }> = {
  "1am": { num: "1", bg: "#DBEAFE", color: "#1E40AF" },
  "2am": { num: "2", bg: "#FEF3C7", color: "#92400E" },
  "3am": { num: "3", bg: "#EDE9FE", color: "#4C1D95" },
  "4am": { num: "4", bg: "#FCE7F3", color: "#9D174D" },
};


// ─── Page ────────────────────────────────────────────────────────────────────

export default async function EnsCmPage() {
  const data = await enspri("ens-cm");

  const stages = data
  const certs = await finalbac('ens-cm/4am')
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 py-12">
        ...
        <div className="mb-12">
          ...
          <p className="text-base max-w-md leading-relaxed" style={{ color: "#777" }}>
            دروس واختبارات وفروض لجميع سنوات التعليم المتوسط
          </p>
        </div>
