import { cinqEmeYearScraper } from "@/scraper/finals";
...
// Helper to generate consistent colors if the scraper doesn't provide them
const getColor = (str: string) => {
  const colors = [
    "#2563eb", "#e11d48", "#0891b2", "#dc2626", "#7c3aed", 
    "#16a34a", "#059669", "#d97706", "#0f766e", "#ea580c"
  ];
  let hash = 0;
...
export default async function CinqEmeYearPage({ params }: PageProps) {
  const { year } = await params;
  const data = await cinqEmeYearScraper(year);

  // Map the scraper output to match the expected UI structure seamlessly
  const subjects = (data.subjects || []).map((item: any) => ({
    title: item.subject || item.title || "مادة",
    subtitle: "الموضوع والتصحيح",
    color: item.color || getColor(item.subject || item.title || ""),
    icon: item.icon || null,
    topic: item.exam || item.topic,
    correction: item.correction,
    detailedCorrection: item.detailedCorrection || null,
  }));

  const totalTopics = subjects.filter((s) => s.topic).length;
  const totalCorrections = subjects.filter((s) => s.correction).length;
    ...
    <main className="min-h-screen font-['Tajawal']" style={{ background: "#F7F3EC" }}>
      ...
      <div className="max-w-5xl mx-auto px-6 lg:px-8 py-10">
        ...
        <nav className="flex items-center gap-2 text-xs mb-6" style={{ color: "#AAA" }}>
          ...
          <ChevronLeft className="w-3 h-3" />
          <Link href="/ens-pri/fifth-primary" className="hover:text-[#7C3AED] transition-colors">
            السنة الخامسة
          </Link>
          <ChevronLeft className="w-3 h-3" />
          <span className="font-semibold" style={{ color: "#7C3AED" }}>شهادة {year}</span>
        </nav>
        ...
        <div className="mb-6">
          ...
          >
            {data.title || `مواضيع وحلول شهادة التعليم الابتدائي ${year}`}
          </h1>
          {data.description && (
